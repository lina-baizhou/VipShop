        ...
        <div style="width: 1000rem;height:auto;margin-top: 20rem;display: flex;flex-wrap: wrap;padding-bottom: 20rem">
            <div style="width: 1000rem;height: auto;border: 1px solid #dfdfdf;margin-top: 10rem;padding:0rem 20rem;display: flex;justify-content: space-between;align-items: flex-start">
                ...
                <div style="width: 771rem;height: auto;display: flex;justify-content: flex-start;align-items: center;flex-wrap: wrap">
                    <span class="chooseType"  @click="chooseType(item,index)" v-for="(item,index) in store.state.searchedTypes.list" :key="index" style="cursor: pointer;width: auto;height: 50rem;margin-right: 50rem;display: flex;align-items: center;justify-content: flex-start;color: #666;font: 14rem tahoma,arial,Hiragino Sans GB,WenQuanYi Micro Hei,\5FAE\8F6F\96C5\9ED1,\5B8B\4F53,sans-serif;">
                        {{item.type}}
                    </span>
                </div>
            ...
            <div style="width: 100%;height: auto;display: flex;justify-content: flex-start;align-items: flex-start;flex-wrap: wrap">
                <div v-for="(item,index) in  store.state.searchedGoodsBySearchItem.list" :key="index" style="width: 25%;height: auto;display: flex;justify-content: center">
                    <goods>
                        <template #goodsImg>
  ...
  export default {
    ...
    setup(){
      ...
      function  clear() {
        ...
        searchGoods(0,0,store.state.searchItem,typeId,lastColumn,lastFlag,currentPage).then(res=>{
          store.state.page.data=res.data
          store.state.searchedGoodsBySearchItem.list=res.data.records;
      ...
      function fix() {
        ...
        searchGoods(minPrice.value,maxPrice.value,store.state.searchItem,typeId,lastColumn,lastFlag,currentPage).then(res=>{
          store.state.page.data=res.data
          store.state.searchedGoodsBySearchItem.list=res.data.records;
      ...
      function chooseSelectItem(index){
        ...
        if(index==2){
          if(lastIndex!=index){
            ...
            searchGoods(minPrice.value,maxPrice.value,store.state.searchItem,typeId,'goodsPrice',1,currentPage).then(res=>{
              store.state.page.data=res.data
              store.state.searchedGoodsBySearchItem.list=res.data.records;
            ...
            if(downFlag.value==true){//根据价格升序
              ...
              searchGoods(minPrice.value,maxPrice.value,store.state.searchItem,typeId,'goodsPrice',0,currentPage).then(res=>{
                store.state.page.data=res.data
                store.state.searchedGoodsBySearchItem.list=res.data.records;
        ...
        if(index==1){
          ...
          searchGoods(minPrice.value,maxPrice.value,store.state.searchItem,typeId,'goodsSalesVolumn',0,currentPage).then(res=>{
            store.state.page.data=res.data
            store.state.searchedGoodsBySearchItem.list=res.data.records;
        ...
        if(index==0){
          lastFlag=0
          lastColumn=''
          searchGoods(minPrice.value,maxPrice.value,store.state.searchItem,typeId,'',0,currentPage).then(res=>{
            store.state.page.data=res.data
            store.state.searchedGoodsBySearchItem.list=res.data.records;
            for(let i=0;i<store.state.searchedGoodsBySearchItem.list.length;i++){
              console.log(store.state.searchedGoodsBySearchItem.list[i].goodsImage1);
              store.state.searchedGoodsBySearchItem.list[i].goodsImage1=require("../../assets/img/"+store.state.searchedGoodsBySearchItem.list[i].goodsImage1);
            }
          })
        }
      ...
      function chooseType(item,index) {
        ...
        maxPrice.value=0
        chooseSelectItem(0)
        for(let i=0;i<store.state.searchedTypes.list.length;i++){
          $('.chooseType').eq(i).css('color', ' #666')
        }
        $('.chooseType').eq(index).css('color', 'rgb(242,2,128)')
      }

      console.log(store.state.searchedGoodsBySearchItem.list);
