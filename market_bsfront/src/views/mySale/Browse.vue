    ...
    <div style="width: 100%;height: auto;display: flex;justify-content:center;align-items: center;flex-wrap: wrap" class="mainDiv">
        <div style="width: 100%;height: 55rem;border-bottom: 1rem solid #e9e9e9;display: flex;justify-content: center" class="mainDiv">
            <div style="width:1000rem;height: 100%;display: flex;justify-content: space-between;align-items: center">
                <div style="width: 333rem;height: 100%;display: flex;justify-content: center;align-items: center">
                    <router-link to="/mainHeader/mySale/collection" style="width: 100%;display: flex;justify-content: center;align-items: center;border-right: 1rem solid #e9e9e9;color: #808080;font-size: 16rem;font-family: tahoma, arial, 'Hiragino Sans GB', 'WenQuanYi Micro Hei', 微软雅黑, 宋体, sans-serif;">
                        商品收藏
                    </router-link>
                </div>
                <div style="width: 333rem;height: 100%;display: flex;justify-content: center;align-items: center">
        ...
        <div style="width: 100%;height: auto;display: flex;justify-content: center" class="mainDiv">
            <div style="width: 1030rem;height: auto;display: flex;justify-content: flex-start;align-items: flex-start;flex-wrap: wrap;margin-top: 20rem">
                <div v-for="(item,index) in store.state.goodsBrowseList.list" :key="index" @mouseenter="enterDiv(index)" @mouseleave="leaveDiv(index)" style="width: 233rem;height: 355rem;border:1rem lightgrey solid;margin: 0 20rem 20rem 0;display: flex;flex-direction: column;align-items: flex-start;justify-content: flex-start">
...
<script>
  ...
  import selectBrowsingHistoryList from "../../Internet/home/browsingHistory/select/selectBrowsingHistoryList";
  import $ from "jquery"
  import removeBrowsingHistory from "../../Internet/home/browsingHistory/remove/removeBrowsingHistory";
  ...
  export default {
    ...
    setup(){
      ...
      selectBrowsingHistoryList(JSON.parse(sessionStorage.getItem('user')).userId).then(res=>{
        store.state.goodsBrowseList.list=res.data
        for(let i=0;i< store.state.goodsBrowseList.list.length;i++){
          store.state.goodsBrowseList.list[i].goodsImage1=require('../../assets/img/'+ store.state.goodsBrowseList.list[i].goodsImage1)
        }
      })
      ...
      function deleteBrowse(item,index){
        removeBrowsingHistory(JSON.parse(sessionStorage.getItem('user')).userId,item.goodsId).then(res=>{
          if(res.code==0){
            store.state.goodsBrowseList.list.splice(index,1)
          }
        })
      }
