...
<script>
  ...
  import saveGoodsSpecifications from "../../../Internet/home/goodsSpecifications/save/saveGoodsSpecifications";
    ...
    setup(){
        ...
        if(goodsSpecifications.list2.length-2>0) {
          for (let i = 1; i < goodsSpecifications.list1.length - 1; i++) {
            for (let j = 1; j < goodsSpecifications.list2.length-1; j++) {
              let goodsSpecification = reactive({
      ...
      function releaseAGoods() {
        ...
        if(flag==true){
          ...
          instance.post('/servlet/goods/addAGoods',formData).then(res=>{
            console.log(res);
            if(res.data.code==0){
              for(let i=0;i<goodsSpecifications1.list.length;i++){
              ...
              saveGoodsSpecifications(goodsSpecifications1).then(res=>{
                console.log(res);
                if(res.code==0) {
                  alert('上传商品成功')
                }
